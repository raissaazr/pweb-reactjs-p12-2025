  ...
  try {
    ...
    const orders = await prisma.order.findMany({
      include: {
        ...
        items: { // Relation name in Order model is 'items'
          include: {
...
router.get('/statistics', async (req: Request, res: Response) => { // Keep path '/statistics'
  try {
    ...
    for (const sale of genreSales) {
      ...
      if (book && book.genre) {
        ...
        salesByGenre[genreName] =
          (salesByGenre[genreName] || 0) + (sale._sum.quantity || 0);
      } else {
 ...
 * @access  Public/Private
 */
router.get('/:id', async (req: Request, res: Response) => { // Updated path to :id
  try {
    ...
    const itemsResponse = order.items.map(item => ({
        bookId: item.bookId,
    ...
    const totalPrice = itemsResponse.reduce((sum, item) => sum + item.subtotalPrice, 0);
